...
* **Static Analysis & Tooling:** Production enforcement via `mypy` for strong typing and `ruff` for continuous code quality and linting.

### 4.3 Layer 1 — The Neural Extraction Interface ("The Eye")
During chapter ingestion, each text passage is submitted to the LLM endpoint under a tightly constrained prompt schema. The model is not asked to summarize or interpret—it is instructed to function strictly as a Named Entity Recognizer (NER) and profile extractor. The output is a rigid JSON object containing active characters (along with dynamically inferred profile traits such as *gender* and *personality*) and active world terms. This constraint dramatically reduces token consumption per call and eliminates the risk of unbounded narrative interpretation that would otherwise require large context windows.

In the event of API unavailability or cost-constrained operation, the deterministic spaCy NLP pipeline activates as a lossless fallback. While less accurate on unconventional fantasy nomenclature (see Section 6.1), this fallback ensures continuous system operation without external dependencies.
...
$$ Score(c) = \text{PageRank}(c) \times N \times (1 - \lambda)^{\Delta t} $$

Where the variables are defined as follows:
* **$c$** — the specific Character Node within the directed graph.
* **$\text{PageRank}(c)$** — the structural centrality score calculated via standard matrix transformation on the current DAG.
* **$N$** — the total number of nodes in the graph (N-scaling factor). Normalizing by total nodes ensures that calculated scores reflect absolute centrality regardless of increasing graph density over time.
* **$\lambda$** — the Temporal Decay Rate (bounded between 0 and 1; default value 0.15). This parameter governs the aggressiveness of narrative forgetting: higher values produce faster relevance decay. The default value of $\lambda = 0.15$ aligns the decay slope with observed pacing conventions in serialized web fiction arcs.
...
This formulation ensures that a character's score peaks during their active narrative arc and degrades deterministically as chapters pass without their appearance. When $Score(c)$ crosses an upper-bound threshold (`MAIN_CAST_THRESHOLD` = 5.0), the Audio Broker layer assigns a premium, persistent TTS Voice ID to that character. During this critical graduation check, the system executes an automated merge operation: it retrieves the latest `wiki_traits` (including the Neural layer's identified gender) and injects them alongside runtime vocal states, ensuring that Voice ID assignment relies on fully synthesized lore and narrative reality. When the score falls below a lower bound hysteresis threshold ($\delta_{lower} = 0.3$), the Voice ID is released back to the generic pool. Critically, it solves the Casting Paradox by making voice assignment a continuously re-evaluated, evidence-based decision rather than a permanent, speculative one at first appearance.

### 4.6 Voice Broker Mechanics & The Bootstrapping Problem
When $Score_{char}$ crosses the persistent `MAIN_CAST` threshold, the Voice Broker assigns a persistent TTS Voice ID from a finite pool of curated voices (6 Kokoro models). If the score subsequently decays beneath the $\delta_{lower}$ boundary, the Voice ID is recycled back to the generic pool.

However, this generates a bootstrapping paradox: entirely new characters possess no topological significance upon introduction, causing the Casting Paradox to re-emerge in early chapters. To mathematically mitigate this, the architecture conditionally applies a **Debut Prominence Quotient (DPQ)**. The DPQ calculates a character's localized dominance of interaction edges exclusively within the graph subset generated by the single chapter of their introduction. If a newly discovered character commands $\ge 40\%$ of the weighted narrative action in their debut chapter (i.e. $DPQ \ge 0.40$), they are granted a provisional graduation (assigned a score bound of $\delta_{upper} + 0.01$). 

If they abruptly vanish from the narrative in subsequent chapters, the standard $(1-\lambda)^{\Delta t}$ Temporal Decay will gracefully erode this provisional score, triggering a de-graduation back beneath $\delta_{upper}$ and recovering the Voice ID. This mechanism allows the system to instantly respond to new protagonists dynamically without hardcoding heuristic bypasses.
...
| **Litellm (Neural Engine)** | 100.0% | 100.0% | **100.0%** | **84.0%** |

The performance differential between the two pipelines is stark. The spaCy fallback failed to meet the target threshold, achieving a Combined F1 of only 46.0%—driven in part by critically low World Terms accuracy. This reflects a fundamental constraint: deterministic NLP heuristics calibrated for conventional English prose systematically misclassify invented proper nouns such as *Zorian* and *Kirielle* as sentence-initial common nouns. The LiteLLM Neural pipeline, by contrast, achieved perfect 100% Character Entity F1 and an 84.0% Combined F1, demonstrating the necessity of contextual understanding for extraction.
...
To definitively prove that the decay mechanism functions as intended, the system was ablated against a static frequency baseline ($\lambda=0.0$) over a multi-chapter simulation shown in Section 1. This single comparison proves the mathematical divergence of the architecture. Fine-grained optimization comparing varying non-zero decay rates (e.g., $\lambda=0.05$ vs $\lambda=0.20$) over longitudinal datasets remains an area for future work.
...
### 6.3 Addressed Limitations and Operational Constraints
The system architecture encountered several critical limitations during its proof-of-concept phase. Through Phase 6 engineering, these vulnerabilities were aggressively targeted and remediated, leaving only the primary operational constraint regarding cloud dependence.

#### 6.3.1 Ongoing Constraint: The "Zero-Local-GPU" Compromise
While the system successfully isolates local hardware from massive VRAM requirements, it remains entirely dependent on external cloud GPUs to function accurately. The Neural Extraction Interface requires the heavy cognitive capabilities of macroscopic LLMs (Groq Llama-3-70b and Google Gemini 1.5 Flash). When the system was forced to fall back on its truly localized, CPU-bound extraction pipeline (`spaCy`), it achieved only a 40.0% Character F1 score, demonstrating an absolute operational reliance on high-uptime external APIs.

#### 6.3.2 Remediated Flaws (Phase 6 Finalization)
1. **Unverified Temporal Algorithm**: The original evaluation relied blindly on single-chapter tests where $\Delta t = 0$. By integrating a longitudinal, multi-chapter sequential evaluation harness, the Temporal Decay mechanism was empirically proven to diverge from static frequency baselines, confirming characters accurately "fade" from system memory without interaction.
2. **Hardcoded Bootstrapping Heuristics**: The early system bypassed its own algorithm by assigning automatic importance to the "first 5 characters". This was stripped out and replaced with the **Debut Prominence Quotient (DPQ)**—a mathematically sound graph subset formula that calculates and promotes dynamic dominance strictly in a character's foundational chapter.
3. **Analytically Shallow Edges**: The original Binary DAG lacked qualitative nuance (a murder had the same weight as a greeting). The prompt schema was augmented with an empirical 1-5 intensity rubric, transforming the structure into a fully **Weighted PageRank Graph** that allows authority to concentrate naturally around highly impactful plot elements. 
4. **Perceptual Validation Blindspot**: Despite synthesizing an audio UI, the original research lacked human validation. A blind A/B audio testing Streamlit application was developed to capture **Mean Opinion Scores (MOS)**, randomizing clip order completely to eliminate presentation bias while tracking distinctiveness, naturalness, and fatigue.
...
In summary, the architecture successfully proves that separating semantic extraction (using LLMs) from structural logic (using local graphs) is computationally efficient and conceptually robust. By empirically verifying the divergence of Temporal PageRank across multi-chapter tracking tests, the system definitively demonstrates its ability to model the forward flow of narrative engagement at scale.
...
[5]  Reid, M., Savinov, N., Denisov, D., et al. (2024). Gemini 1.5: Unlocking multimodal understanding across millions of tokens of context. *arXiv preprint arXiv:2403.05530*.

[6]  Bain, M., Huh, J., Han, T., & Zisserman, A. (2023). WhisperX: Time-accurate speech transcription of long-form audio. *INTERSPEECH 2023*.

[7]  Vala, H., Jurgens, D., Piper, A., & Ruths, D. (2015). Mr. Bennet, his coachman, and the Archbishop walk into a bar but only one of them gets recognized: On the difficulty of detecting characters in literary texts. *Proceedings of the 2015 Conference on Empirical Methods in Natural Language Processing*.
