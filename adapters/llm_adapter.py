...
def analyze_text(text: str, model: str = "groq/llama-3.1-8b-instant"):
    ...
    for attempt in range(1, 4):  # Retry up to 3 times
        try:
            ...
            last_error = e
            logger.error(f"Error calling LLM {model} (attempt {attempt}/3): {e}")
            if attempt < 3:
...
def analyze_text_json(text: str, model: str = "groq/llama-3.1-8b-instant") -> dict:
    ...
    for attempt in range(1, 4):  # Retry up to 3 times
        try:
            ...
            last_error = e
            logger.error(f"Error calling LLM {model} for JSON (attempt {attempt}/3): {e}")
            if attempt < 3:
                time.sleep(2 ** attempt)  # Exponential backoff: 2s, 4s

    logger.error(f"LLM Extraction failed after 3 attempts: {str(last_error)}")
    return {"error": f"API Fallback: ... {str(last_error)}"}
