...
> "There's a retry system that keeps re-trying failed work... The only thing using it is the daily email job, which runs once a day. Want to keep it, or rip it out? *(file: [src/queue/retry.ts](src/queue/retry.ts))*
> ```ts
> // src/queue/retry.ts
> export function retryWithBackoff() { ... }
> ```"

> "There's a price-formatting helper that takes 5 different settings... But every place that uses it ignores all 5 settings. Want to simplify it down? *(file: [src/format.ts](src/format.ts))*
> ```ts
> // src/format.ts
> export function formatPrice(amount, currency, lang, dec, etc) { ... }
> ```"
...
- Always include a 3-5 line code snippet proving the issue exists to build trust.
