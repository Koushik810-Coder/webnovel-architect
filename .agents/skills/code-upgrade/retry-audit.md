...
**Why it matters:** real money lost on every retry.
  ...
  *(file: [src/webhooks/stripe.ts:42](src/webhooks/stripe.ts#L42))*
  ```ts
  // No deduplication check before granting credits
  await user.incrementCredits(event.amount);
  ```
...
- Explicitly check if the operation being retried has a unique constraint, dedup key, or idempotency key. If it doesn't, flag it as dangerous.
- Always include a 3-5 line code snippet showing the lack of idempotency constraints.
